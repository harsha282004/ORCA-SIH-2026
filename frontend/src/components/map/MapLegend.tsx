...
function Swatch({ color, shape = "square" }: { color: string; shape?: "square" | "circle" | "line" }) {
  ...
  return (
    <span
      className={`inline-block h-3 w-3 border border-white/20 ${shape === "circle" ? "rounded-full" : "rounded-sm"}`}
...
function Row({ color, label, shape }: { color: string; label: string; shape?: "square" | "circle" | "line" }) {
  return (
    <div className="flex items-center gap-2 text-xs text-marine-white/80">
        ...
        <div className="max-h-[60vh] space-y-3 overflow-y-auto px-3 pb-3 text-xs">
          {show("risk") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Risk (deterministic ORCA Risk Engine)</p>
          ...
          {show("fishing") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Fishing</p>
              ...
              <p className="pt-1 text-[10px] italic text-marine-white/40">
          ...
          {show("safety") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Safety</p>
          ...
          {show("hazards") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Marine Hazards (Phase 4 — real detected hazards)</p>
          ...
          {show("marine") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Marine (real Open-Meteo samples)</p>
          ...
          {show("reference") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Reference (GEBCO / INCOIS)</p>
              ...
              <p className="pt-1 text-[10px] italic text-marine-white/40">GEBCO bathymetry is reference/supporting data, not a navigation safety guarantee.</p>
            </section>
          )}

          {show("route") && (
            <section className="space-y-1">
              <p className="font-semibold text-marine-white/50">Route</p>
